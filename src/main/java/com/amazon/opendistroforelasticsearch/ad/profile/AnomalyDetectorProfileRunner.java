...
package com.amazon.opendistroforelasticsearch.ad.profile;
...
public class AnomalyDetectorProfileRunner {
        ...
        return ActionListener.wrap(getResponse -> {
            if (getResponse != null && getResponse.isExists()) {
                try (
                    ...
                    listener.failImmediately(new RuntimeException("Fail to parse detector with id: " + detectorId, e));
                }
            } else {
                listener.failImmediately("Fail to find detector with id: " + detectorId);
            }
        }, exception -> {
            listener.failImmediately("Fail to find detector with id: " + detectorId, exception);
        });
    }
    ...
    private ActionListener<SearchResponse> onGetLatestAnomalyResult(
