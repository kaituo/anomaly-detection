...
import com.amazon.opendistroforelasticsearch.ad.profile.AnomalyDetectorProfileRunner;
...
public class RestGetAnomalyDetectorAction extends BaseRestHandler {
    ...
    protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
        ...
        if (!Strings.isEmpty(typesStr) || rawPath.endsWith(PROFILE) || rawPath.endsWith(PROFILE + "/")) {
            return channel -> profileRunner.profile(
                detectorId,
                getProfileActionListener(channel, detectorId),
                getProfilesToCollect(typesStr)
            );
        } else {
            MultiGetRequest.Item adItem = new MultiGetRequest.Item(ANOMALY_DETECTORS_INDEX, detectorId).version(
                RestActions.parseVersion(request)
            );
            MultiGetRequest multiGetRequest = new MultiGetRequest().add(adItem);
            if (returnJob) {
                MultiGetRequest.Item adJobItem = new MultiGetRequest.Item(ANOMALY_DETECTOR_JOB_INDEX, detectorId).version(
                    RestActions.parseVersion(request)
                );
