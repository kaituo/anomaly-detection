...
public class AnomalyDetectorProfileRunnerTests extends ESTestCase {
    ...
    private void setUpClientGet(
        ...
        StateIndexStatus stateIndexStatus
    ) throws IOException {
        ...
        doAnswer(invocation -> {
            ...
            if (request.index().equals(ANOMALY_DETECTORS_INDEX)) {
                ...
                if (errorResultStatus == ErrorResultStatus.INDEX_NOT_EXIT || stateIndexStatus == StateIndexStatus.INIT_NOT_EXIT) {
                ...
                switch (stateIndexStatus) {
                    ...
                    case EXCEPTION:
                        ...
                        break;
                    case NOT_UPDATED:
                        result.lastUpdateTime(Instant.ofEpochMilli(1));
                        break;
                    default:
                        assertTrue("should not reach here", false);
                        break;
    ...
    public void testDetectorNotExist() throws IOException, InterruptedException {
        setUpClientGet(
            DetectorStatus.INDEX_NOT_EXIST,
            JobStatus.INDEX_NOT_EXIT,
            RCFPollingStatus.EMPTY,
            ErrorResultStatus.NO_ERROR,
            StateIndexStatus.EMPTY
        );
    ...
    public void testDisabledJobIndexTemplate(JobStatus status) throws IOException, InterruptedException {
        setUpClientGet(DetectorStatus.EXIST, status, RCFPollingStatus.EMPTY, ErrorResultStatus.NO_ERROR, StateIndexStatus.EMPTY);
    ...
    public void testInitOrRunningStateTemplate(RCFPollingStatus status, DetectorState expectedState) throws IOException,
        ...
        setUpClientGet(DetectorStatus.EXIST, JobStatus.ENABLED, status, ErrorResultStatus.NO_ERROR, StateIndexStatus.EMPTY);
    ...
    public void testErrorStateTemplate(
        ...
        InterruptedException {
        setUpClientGet(DetectorStatus.EXIST, jobStatus, initStatus, status, StateIndexStatus.EMPTY);
        DetectorProfile expectedProfile = new DetectorProfile.Builder().state(state).error(error).build();
        final CountDownLatch inProgressLatch = new CountDownLatch(1);
    ...
    public void testInitNoError() throws IOException, InterruptedException {
        testErrorStateTemplate(
            RCFPollingStatus.INIT_NOT_EXIT,
    ...
    public void testExceptionOnStateFetching() throws IOException, InterruptedException {
        setUpClientGet(
            DetectorStatus.EXIST,
            JobStatus.ENABLED,
            RCFPollingStatus.EXCEPTION,
            ErrorResultStatus.NO_ERROR,
            StateIndexStatus.EMPTY
        );
    ...
    public void testProfileModels() throws InterruptedException, IOException {
        setUpClientGet(DetectorStatus.EXIST, JobStatus.ENABLED, RCFPollingStatus.EMPTY, ErrorResultStatus.NO_ERROR, StateIndexStatus.EMPTY);
